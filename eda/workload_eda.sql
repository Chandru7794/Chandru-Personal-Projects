...
SELECT
    ...
    COUNT(*)
FROM workload
GROUP BY video_subtype
ORDER BY video_subtype;
...
SELECT * FROM workload
WHERE video_subtype IN ('Multiple', 'N/A');
...
SELECT
    ...
    ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM workload_dates_filled), 2)
        AS pct_of_total
FROM workload_dates_filled
...
FROM ordered AS o
WHERE
    o.rn IN (SELECT rn FROM midnight_rns)
...
SELECT
    COUNT(*) FILTER (
        WHERE CASE WHEN duration < 0 THEN duration + 1440 ELSE duration END <= 0
    ) AS zero_duration,
    COUNT(*) FILTER (
        WHERE CASE WHEN duration < 0 THEN duration + 1440 ELSE duration END
        > 240
    ) AS over_4hrs
FROM workload_durations_fixed;
    ...
    SELECT
        *,
        SUM(
            CASE
                WHEN
                    gap_from_prev_minutes IS NULL OR gap_from_prev_minutes > 15
                    THEN 1
                ELSE 0
            END
        )
...
SELECT
    ...
    ROUND(
        SUM(sessions_combined) * 100.0 / (SELECT COUNT(*) FROM workload_deduped
        ),
        2
...
WITH grp AS (
    ...
    HAVING COUNT(*) >= 2
    ORDER BY
        sessions_combined DESC,
        media_title ASC,
        video_type ASC,
        video_subtype ASC,
        creation_category ASC,
        island_id ASC
    LIMIT 10
)
...
FROM workload_islands AS wi
INNER JOIN grp
    ON
        wi.media_title = grp.media_title
...
ORDER BY
    grp.sessions_combined DESC, wi.media_title ASC, wi.video_type ASC, wi.video_subtype ASC, wi.creation_category ASC, wi.island_id ASC, wi.ts_start ASC;
